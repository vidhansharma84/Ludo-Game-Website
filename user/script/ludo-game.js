...
function rollDiceAnimation(idValue) {
  ...
  // }
  // Step 2: Listen for dice result from server
  socket.once("diceResult", (data) => {
    clearInterval(timerId);
    rndmNo = data.diceValue;
    console.log(rndmNo);
    showDice(idValue, rndmNo);

    if (rndmNo == 5 && countSix != 3) {
      if (players[playerName].outArea.length == 0 && players[playerName].inArea.length > 0) {
        openPawn();  // Auto open
      } else {
        openPawn();  // Manually open
        movePawnOnOutArea();
        updatePlayer();
      }
    } else if (rndmNo < 5) {
...
function rollDice(idValue) {
  socket.emit("rollDice", { roomId, p_id }); 
  // let pX = 0;
  // let pY = 0;
...
function showDice(idValue,rndmNo) {
  let pX = null;
  let pY = null;
  const pXpYarr = [
    [0, 0],
...
function joinGame() {
    ...
    console.log("Game Socket: " + socket + " Player Name: " + p_name);
    
    if (p_name) {
        socket.emit("joinGame", { roomId, p_name,p_id });
    }

    socket.on("gameStarted",(players)=>{
      const diceElements = {
        rPlayer: document.getElementById("redDice"),
        gPlayer: document.getElementById("greenDice"),
        yPlayer: document.getElementById("yellowDice"),
        bPlayer: document.getElementById("blueDice")
      };
      const playerNameColor = {
        rPlayer: document.getElementById("redPlayerName"),
        gPlayer: document.getElementById("greenPlayerName"),
        yPlayer: document.getElementById("yellowPlayerName"),
        bPlayer: document.getElementById("bluePlayerName")
      };
      players.players.forEach(player=>{
        console.log("Player id : " + player.id + " p_id : " + p_id)
        if(player.id != p_id){
          playerNameColor[player.color].textContent=player.name;
          diceElements[player.color].classList.add("disabled");
          diceElements[player.color].style.pointerEvents = "none";
          diceElements[player.color].style.opacity = "0.5";
        }else{
          playerNameColor[player.color].textContent="You";
        }
      })
      console.log("Waiting for others");
    })

    socket.on("waitingForOthers",(players)=>{
      // console.log(players.length);
    })

    socket.on("rollingDice",(rollingDiceData)=>{
      rollDiceAnimation(rollingDiceData.color_animation_code);
    })
